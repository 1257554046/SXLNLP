...
def find_similar_item(user_to_rating):
    ...
    # 复用一下下方的相似度计算方法
    return find_similar_user(item_to_vector, 1000)


# 依照user对item的打分判断user之间的相似度
def find_similar_user(user_to_rating, limit_count=100):
    ...
    for user_a, ratings_a in user_to_rating.items():
        ...
        for user_b, ratings_b in user_to_rating.items():
            # 全算比较慢，省去一部分用户
            if user_b == user_a or user_b > limit_count or user_a > limit_count:
                continue
            # ab用户互换不用重新计算cos
...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    for similar_item, similarity in similar_items[item_id][:topn]:
        # 该用户对相似电影的打分
        similar_item_rating = user_to_rating[user_id][similar_item - 1]
        # 分数*item相似度，作为一种对分数的加权，越相似的item评分越重要
        pred_score += similar_item_rating * similarity
        # 如果这个相似item没看过，就不计算在总数内
