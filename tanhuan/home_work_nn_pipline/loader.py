...
class DataGenerator:
    ...
    def load(self):
        ...
        # 计算要获取的数据行数
        num_rows = int(len(df) * self.config["train_rate"])
        # 获取训练集合数据
        if self.data_type == 1:
            df_data = df.iloc[:num_rows, :2]
        # 获取验证集合数据
        else:
            df_data = df.iloc[num_rows:, :2]
        for line in df_data.values:
            ...
            label_index = torch.LongTensor([line[0]])
...
def load_data(data_path, config, data_type, shuffle=True):
