...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.layer_norm = nn.LayerNorm(sentence_length)  # 层归一化
        self.dp_layer = torch.nn.Dropout(0.1)
    ...
    def forward(self, x, y=None):
        ...
        _, x = self.layer(x)                #(batch_size, 1) -> (batch_size, 1)
        x = x.squeeze()
        x = self.layer_norm(x)
        x = self.classify(x)
        x = self.layer_norm(x)
        y_pred = self.dp_layer(x)
        if y is not None:
...
def build_sample(vocab, sentence_length):
    ...
    # 获取随机索引，插入a
    y = np.random.randint(0, sentence_length)
    x[y] = "a"
...
if __name__ == "__main__":
    main()
    # test_strings = ["anvfee", "casdfg", "cqadeg", "nakwwa"]
